...
public class CloudStorageHelper {
    ...
    static {
        ...
        try {
            serviceAccount = new ClassPathResource ("se331imageupload.json").getInputStream();
            storage = StorageOptions.newBuilder()
            .setCredentials (GoogleCredentials.fromStream (serviceAccount))
            .setProjectId("se331imageupload") .build().getService();
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    ...
    public String uploadFile (MultipartFile filePart, final String bucketName) throws IOException {
        SimpleDateFormat sdf = new SimpleDateFormat ("yyyy-MM-dd HHmmssSSS"); String dtString = sdf.format (new Date());
    ...
    public String getImageUrl(MultipartFile file, final String bucket) throws IOException, ServletException {
        ...
        if(fileName != null && !fileName.isEmpty() && fileName.contains(".")){
            ...
            String[] allowedExit = {"jpg", "jpeg", "png", "gif"};
            for(String s : allowedExit){
